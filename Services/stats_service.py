...
def CreateStatRows(points: list[Type[Point]], multiVehicle: bool, isTest: bool) -> pd.DataFrame:
    ...
    for vehicle in vehicles:
        ...
        stat_dataframe_rows.append(StatDataRow(vehicle.capacity,
                                               ...
                                               # total_uran_plus,
                                               # total_uran_minus))
    if isTest and multiVehicle:
        return CreateSummaryDataframeRow(stat_dataframe_rows)
    return stat_dataframe_rows


def CreateSummaryDataframeRow(stat_dataframe_rows: list[Type[StatDataRow]]):
    return [StatDataRow(np.mean([s.capacity for s in stat_dataframe_rows]),
                        sum([s.total_dist for s in stat_dataframe_rows]),
                        ...
                        # sum([s.total_orange for s in stat_dataframe_rows]),
                        # sum([s.total_uran for s in stat_dataframe_rows]),
                        # sum([s.total_tuna_plus for s in stat_dataframe_rows]),
                        # sum([s.total_tuna_minus for s in stat_dataframe_rows]),
                        # sum([s.total_orange_plus for s in stat_dataframe_rows]),
                        # sum([s.total_orange_minus for s in stat_dataframe_rows]),
                        # sum([s.total_uran_plus for s in stat_dataframe_rows]),
                        # sum([s.total_uran_minus for s in stat_dataframe_rows]))]


def CreateStatDataframe(points: list[Type[Point]], multiVehicle: bool, isTest: bool):
    stat_dataframe_rows = CreateStatRows(points, multiVehicle, isTest)
